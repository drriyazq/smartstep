...
Sign-in is **WhatsApp-OTP only** (2026-05-20 cutover, matches MedUnity). Every phone number — Indian and international — gets the 6-digit code on WhatsApp via the shared Meta WABA + System User token (`POST /auth/otp/{send,verify}/` — see `api/whatsapp.py` and `api/otp.py`). Successful verifies create or reuse a user via `AppUserPhone(user, phone_e164)`. Users without WhatsApp installed on the sign-up number can't sign in — same posture as MedUnity. Test bypass: phones in `OTP_TEST_PHONES` skip the WhatsApp send and accept `OTP_TEST_CODE` directly. Existing pre-OTP accounts (`firebase_<google_uid>` legacy usernames) can be re-claimed by phone via `python manage.py link_user_phone <username_or_email> <+91…>`.

The old Firebase Phone Auth fallback path was dropped: `/auth/firebase/` no longer exists, `firebase_auth` is no longer in `pubspec.yaml`, and the `firebase-credentials.json` file is unreferenced (left on disk in case we ever re-enable). `firebase_core` + `firebase_crashlytics` + `firebase_messaging` + `firebase_analytics` stay — they cover crash reports, push, and analytics, none of which need the auth SDK. SHA-1 fingerprint configuration is now irrelevant to sign-in.
